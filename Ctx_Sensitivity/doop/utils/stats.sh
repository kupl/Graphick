...
function run-stats()
{
	...
	$bloxbatch -db $database -execute "+Stats:Runtime($elapsedTime, \"loading statistics (simple) time\")."
        ...
        echo $graph
	if [ "$graph" = "true" ]; then 
	    preprocess $DOOP_HOME/logic/library/graph-heuristic.logic $DOOP_HOME/tmp/graph-heuristic.logic
	    echo -n "loading graph heuristic (simple) "
	    timing $bloxbatch -db $database -addBlock -file $DOOP_HOME/tmp/graph-heuristic.logic
