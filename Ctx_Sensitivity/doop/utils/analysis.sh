...
function create-database()
{
    ...
    do
        ...
        if [[ -e $cachefacts ]] && flag-isset cache; then
            ...
            # Run Soot
	    if [ "$data" = "true" ]; then 
              timing java -cp ${SOOT_CLASSES}:${SOOT_FACT_GEN_STMT} Main \
	    ...
	    echo $data
	    if [ "$data" = "true" ]; then 
                ${DOOP_HOME}/gen-import_tmp -csv $DOOP_HOME/tmp/fact-declarations.import
	    else
	    ...
	    echo $data
	    if [ "$data" = "true" ]; then 
                ${DOOP_HOME}/gen-import_tmp -text $DOOP_HOME/tmp/fact-declarations.import
	    else
...
EOF
    ...
    # Read Heaps 
                        cat > tmp/CanHeap2type.import <<EOF
option,delimiter,"\t"
option,hasColumnNames,false

fromFile,"$(pwd)/CanHeap2type.facts",h,heap
toPredicate,CanHeap2type,heap
EOF
                $bloxbatch -db $database -import $(pwd)/tmp/CanHeap2type.import

    
                        cat > tmp/CanHeap2obj.import <<EOF
option,delimiter,"\t"
option,hasColumnNames,false


fromFile,"$(pwd)/CanHeap2obj.facts",h,heap
toPredicate,CanHeap2obj,heap
EOF
                $bloxbatch -db $database -import $(pwd)/tmp/CanHeap2obj.import

 
                        cat > tmp/CanHeap1type.import <<EOF
option,delimiter,"\t"
option,hasColumnNames,false

fromFile,"$(pwd)/CanHeap1type.facts",h,heap
toPredicate,CanHeap1type,heap
EOF
                $bloxbatch -db $database -import $(pwd)/tmp/CanHeap1type.import


    # Read Zipper context-sensitive methods
	if [ ! "x$zipper" = "x" ]; then
	#if test "$zipper" = "true"; then
		printf "\nloading ${C_YELLOW}Zipper context-sensitive methods${C_RESET} from %s ...\n" $zipper
		#echo "loading Zipper context-sensitive methods ... "
